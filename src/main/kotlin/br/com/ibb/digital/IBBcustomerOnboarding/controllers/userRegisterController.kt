...
class UserRegisterController (val userRepository: UserRepository) {
    ...
    fun addUser(@RequestBody user : User): ResponseEntity<Any> {
        try{
            ...
            val foundUser: User? = userRepository.findByCpf(user.cpf);
            if(foundUser != null){
                return ResponseEntity(ErrorDTO(HttpStatus.BAD_REQUEST.value(), "Usuário já tem conta no banco"), HttpStatus.BAD_REQUEST)
            }

            val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")

            if(!isDateValid(LocalDate.parse(user.birthdate))) {
                return ResponseEntity(ErrorDTO(HttpStatus.BAD_REQUEST.value(), "Data/formato inváliodo"), HttpStatus.BAD_REQUEST)
            }
            ...
            return ResponseEntity(ErrorDTO(HttpStatus.INTERNAL_SERVER_ERROR.value(),
                "Não foi possivel efetuar o cadastro, tente novamente."), HttpStatus.INTERNAL_SERVER_ERROR)
        }
    }
    ...
    fun checkCpf (@RequestBody dto : cpfDTO) : ResponseEntity<Any> {
        if(dto.cpf.isNullOrBlank() || dto.cpf.isNullOrEmpty()) {
